...
# 1. 젤다의 전설 왕국의 눈물 공식 API 불러오기
print("🌐 API에서 데이터를 불러오는 중...")
url = "https://api.hyrule-compendium.com/v3/compendium/all?game=totk"

# 봇(Bot) 차단을 막기 위해 일반 브라우저인 것처럼 헤더 추가 ( API에 접근하면 차단하기 때문에 우회 해주는 작업 )
headers = {
...
def safe_translate(text):
    ...
    try:
        ...
        print(f"번역 오류 발생 ('{text}'): {e}")
        return text

# 3. 데이터 순회하며 번역 수행
print("모든 텍스트 항목에 대한 번역을 시작")
...
for i, item in enumerate(items):
    ...
    # 리스트 안의 각 단어들을 뽑아내어 하나씩 번역한 뒤 새로운 리스트로 저장
...
print(f"\n모든 항목의 번역이 완료! 바탕화면에 생긴 '{file_name}' 파일을 확인해 보세요.")
